...
这是一个为 `i乐湖` 场景重做的移动端优先前台原型，目标不是后台管理，而是尽量模拟真实在运营中的微信小程序案例库体验。

这次交付是一次重新制作，不是对旧 `project-library/` 轻量 MVP 的换皮延伸。旧目录保留为历史参考，新原型独立放在 `ruc-suzhou-case-library/`。

## 当前范围

- 前台案例展示原型，不接真实 Feishu API
- 本地静态 mock 数据
